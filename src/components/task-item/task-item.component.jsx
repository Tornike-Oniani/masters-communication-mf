...
const TaskItem = ({ data }) => {
  const { id, title, status, resolvedBy, issueDate, description } = data;
      ...
      return (
        <>
          <tr className="table-row">
            ...
            <td className="table-row-item italic">{resolvedBy}</td>
          ...
          <tr className="border-b table-row">
            <td colSpan={6} className="table-row-item">
              ...
              <p>{description}</p>
              <h4 className="font-bold text-lg text-cst-text-gray-800 mb-2 ">
                Comment:
              </h4>
              <div
                className={
                  'border border-gray-400 rounded bg-cst-gray-600 p-2 ' +
                  (resolvedBy !== '' ? '' : 'hidden')
                }
              >
                <p>
              ...
              <textarea
                ...
                rows="4"
                className={
                  'text-input w-full ' + (resolvedBy !== '' ? 'hidden' : '')
                }
                placeholder="Comment"
              />
              <div
                className={'flex mt-2' + (resolvedBy !== '' ? ' hidden' : '')}
              >
                <button className="btn-primary mr-2">Accept</button>
    ...
    return (
      <tr className="grid grid-cols-2 border rounded border-gray-400 shadow mb-3 md:shadow-none md:m-0 md:border-0 md:rounded-none md:border-b md:table-row">
        ...
        <td className="ml-2 mt-2 col-span-2 md:table-row-item">
          ...
          <span className="italic">{resolvedBy}</span>
        </td>
